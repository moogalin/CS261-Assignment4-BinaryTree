...
{
	/* Source Cited: CS 261 Week 5 Lecture Video: BST 3 */
...
{
	/*write this*/
	assert((tree || val) != NULL);

	struct Node * curr = tree->root;
	assert(curr != NULL);

	while (curr != NULL) {
		if (compare(val, curr-> val) == 0) { // Left == Right
			// Value found
			return 1;
		}
		else if (compare(val, curr->val) == 1) { // Left > Right
			curr = curr->right;
		}
		else if (compare(val, curr->val) == -1) { // Left < Right
			curr = curr->left;
...
TYPE _leftMost(struct Node *cur)
{
	...
	//printf("Left most val:\n");
	//print_type(parent->val);
	return parent->val;
}
...
struct Node *_removeLeftMost(struct Node *cur)
{
	...
	struct Node * beforeParent = cur;
	struct Node * parent = cur;
	struct Node * child = cur->left;

	// No left node, left most node is current node
	if (cur->left == NULL) {
		parent = cur->right;
		free(cur);
		return parent;
...
int main(int argc, char *argv[]){	
	...
	pData = (struct data *)malloc(sizeof(struct data)*10);
	pData[0].number = 5;
	pData[0].name = 'Abby';
	pData[1].number = 6;
	pData[1].name = 'Zoey';
	...
	printf("5 vs. 6: %d\n", compare(&pData[0], &pData[1]));
	printf("6 vs. 6: %d\n", compare(&pData[1], &pData[1]));
	printf("6 vs. 5: %d\n", compare(&pData[1], &pData[0]));

	free(pData);

	
	printf("Abby vs. Zoey: %d\n", compare(&pData[0], &pData[1]));
	printf("Abby vs. Abby: %d\n", compare(&pData[1], &pData[1]));
	printf("Zoey vs. Abby: %d\n", compare(&pData[1], &pData[0]));
	printf("Bridge vs. Abby: %d\n", compare(&pData[2], &pData[0]));
	printf("Abby vs. Bridge: %d\n", compare(&pData[0], &pData[2]));
	*/

   //After implementing your code, please uncommemt the following calls to the test functions and test your code 
